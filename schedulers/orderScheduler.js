...
class OrderScheduler {
  ...
  async manualExpireOrders() {
    try {
      logger.info('Manual trigger: Expiring PENDING orders...');
      ...
      if (expiredOrders.length === 0) {
        logger.info('No expired PENDING orders found.');
        return { count: 0, message: 'No expired orders found' };
      }
      ...
      logger.info(`Manual expire completed: ${successCount} orders expired.`);
      return { 
        count: successCount, 
        message: `${successCount} orders expired successfully` 
      };
    } catch (error) {
      logger.error('Error in manual expire:', error);
      throw error;
    }
  }
}
